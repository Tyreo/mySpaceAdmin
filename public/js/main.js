...
window.onload = function () {
  ...
  page.selectBtn.addEventListener("click", function () {
    if (!hasClass(this, "active")) {
      ...
      [].forEach.call(page.checks, function (item, i) {
        item.className.baseVal = item.className.baseVal.replace(
          new RegExp("(\\s|^)check-green(\\s|$)"),
          ""
        );
      });
      [].forEach.call(page.checkbox, function (item, i) {
  ...
  [].forEach.call(page.covers, function (item, index) {
    item.addEventListener("click", function () {
      ...
      if (!check) return;
      if (
        !check.className.baseVal.match(new RegExp("(\\s|^)check-green(\\s|$)"))
      ) {
        check.className.baseVal += " " + "check-green";
      } else {
        check.className.baseVal = check.className.baseVal.replace(
          new RegExp("(\\s|^)check-green(\\s|$)"),
          ""
        );
      }
      ...
      if (hasClass(page.selectBtn, "active")) {
        ...
        [].forEach.call(checked, function (item, i) {
          ...
          send(
            "PUT",
            { type: type },
            "/" + window.location.pathname.split("/")[1] + "/" + _data,
            function () {
        ...
        let _data = item.parentNode.parentNode.dataset.id;
        let albumId = item.parentNode.parentNode.dataset.albumid;
        send(
          "PUT",
          { type: type },
          "/" +
